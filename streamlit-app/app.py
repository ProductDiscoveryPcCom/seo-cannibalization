...
def main():
    ...
    with st.sidebar:
        ...
        cat_file = st.file_uploader(
            ...
            type=['csv'],
            key="categories",
            help="Archivo con columna 'Slug' de categorías principales"
        )
        ...
        adobe_file = st.file_uploader(
            ...
            type=['csv'],
            key="adobe",
            help="Acepta separadores: coma, punto y coma, tab. Columnas: url/page + sessions/visits"
        )
    ...
    def read_csv_flexible(file, name="archivo", skip_comments=False):
        ...
        try:
            ...
            # Saltar líneas de comentarios/metadatos (Adobe Analytics)
            skip_rows = 0
            if skip_comments:
                for i, line in enumerate(lines):
                    line_stripped = line.strip()
                    # Saltar líneas vacías, comentarios (#), o metadatos
                    if (line_stripped.startswith('#') or 
                        line_stripped.startswith('=') or 
                        line_stripped == '' or
                        'Report Suite' in line or
                        'Date Range' in line or
                        'Segment' in line):
                        skip_rows = i + 1
                    else:
                        # Primera línea de datos reales encontrada
                        break
            
            # Encontrar la primera línea con datos reales para detectar separador
            ...
            try:
                ...
                st.error(f"❌ Error al leer {name}: {str(e2)}")
                return None
    
    if gsc_file:
        df = read_csv_flexible(gsc_file, "Search Console")
        if df is not None:
            st.session_state['df'] = df
        
        if cat_file:
            categories_df = read_csv_flexible(cat_file, "Categorías")
            if categories_df is not None:
                st.session_state['categories_df'] = categories_df
        
        if adobe_file:
            adobe_df = read_csv_flexible(adobe_file, "Adobe Analytics", skip_comments=True)
            if adobe_df is not None:
                # Mostrar columnas disponibles para debug
                st.sidebar.caption(f"Columnas Adobe: {', '.join(str(c) for c in adobe_df.columns[:8])}")
                
                # Buscar columnas de URL y sessions (nombres flexibles - Adobe usa muchos nombres)
                url_col = None
                sessions_col = None
                
                url_names = ['url', 'page', 'pagina', 'landing page', 'page url', 'pages', 
                            'landing pages', 'page name', 'pagename', 'entry page']
                sessions_names = ['sessions', 'sesiones', 'visits', 'visitas', 'pageviews', 
                                 'views', 'page views', 'unique visitors', 'visitors',
                                 'entries', 'occurrences', 'instances']
                
                for col in adobe_df.columns:
                    col_lower = str(col).lower().strip()
                    if url_col is None and col_lower in url_names:
                        url_col = col
                    if sessions_col is None and col_lower in sessions_names:
                        sessions_col = col
                
                if url_col and sessions_col:
                    adobe_df = adobe_df.rename(columns={url_col: 'url', sessions_col: 'sessions'})
                    # Limpiar y convertir sessions a numérico
                    ...
                    df['adobe_sessions'] = df['adobe_sessions'].fillna(0).astype(int)
                    st.session_state['df'] = df
                    st.sidebar.success(f"✅ Adobe: {len(adobe_df)} filas cargadas")
                else:
                    st.sidebar.warning(f"⚠️ Adobe: No se encontraron columnas url/sessions")
                    st.sidebar.caption(f"Columnas disponibles: {list(adobe_df.columns)}")
