...
def main():
    ...
    def read_csv_flexible(file, name="archivo"):
        """Lee CSV con detección automática de formato"""
        try:
            # Leer contenido como texto
            content = file.read()
            file.seek(0)  # Reset para posibles re-lecturas
    ...
    if gsc_file:
        ...
        if adobe_file:
            adobe_df = read_csv_flexible(adobe_file, "Adobe Analytics")
            if adobe_df is not None:
                # Mostrar columnas disponibles para debug
                st.sidebar.caption(f"Columnas Adobe: {', '.join(adobe_df.columns[:5])}")
                
                # Buscar columnas de URL y sessions (nombres flexibles)
                url_col = None
                sessions_col = None
                
                for col in adobe_df.columns:
                    col_lower = col.lower().strip()
                    if col_lower in ['url', 'page', 'pagina', 'landing page', 'page url']:
                        url_col = col
                    if col_lower in ['sessions', 'sesiones', 'visits', 'visitas', 'pageviews', 'views']:
                        sessions_col = col
                ...
                if url_col and sessions_col:
                    ...
                    st.sidebar.warning(f"⚠️ Adobe: No se encontraron columnas url/sessions. Columnas: {list(adobe_df.columns)}")
